...
(function (Services) {
    class Records extends services.Services.Core.Service {
        ...
        create(brand, record, recordType = null, user = null, triggerPreSaveTriggers = true, triggerPostSaveTriggers = true) {
        ...
        createInternal(brand, record, packageType, recordType = null, user = null, triggerPreSaveTriggers = true) {
            ...
            if (triggerPreSaveTriggers) {
                ...
                return Rx_1.Observable.fromPromise(obs.then(record => {
                    options.body = record;
        ...
        updateMeta(brand, oid, record, user = null, triggerPreSaveTriggers = true, triggerPostSaveTriggers = true) {
            if (brand == null) {
                return Rx_1.Observable.from(this.updateMetaInternal(brand, oid, record, null, user, false));
            }
            else {
                return RecordTypesService.get(brand, record.metaMetadata.type).flatMap((recordType) => __awaiter(this, void 0, void 0, function* () {
                    let response = yield this.updateMetaInternal(brand, oid, record, recordType, user, triggerPreSaveTriggers);
                    if (triggerPostSaveTriggers) {
                        let resp = response;
        ...
        updateMetaInternal(brand, oid, record, recordType, user = null, triggerPreSaveTriggers = true) {
            return __awaiter(this, void 0, void 0, function* () {
                const options = this.getOptions(sails.config.record.baseUrl.redbox + sails.config.record.api.updateMeta.url, oid);
                if (triggerPreSaveTriggers) {
                    record = yield this.triggerPreSaveTriggers(oid, record, recordType, "onUpdate", user);
                    options.body = record;
                    return yield request[sails.config.record.api.updateMeta.method](options);
                }
                else {
                    options.body = record;
                    sails.log.verbose(util.inspect(options, { showHidden: false, depth: null }));
        ...
        assignPermissions(oid, record, options, user) {
        }
